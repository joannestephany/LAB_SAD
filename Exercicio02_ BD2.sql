...
GO
-- FAZENDO A  QUESTAO DE TRIGGER

CREATE OR ALTER TRIGGER TB_FUNCIONARIO_INSERT
ON TB_FUNCIONARIO
AFTER INSERT
AS
	BEGIN
		...
		SELECT CD_CARGO, CD_ESCOLARIDADE, CD_ESCOLARIDADE FROM INSERTED

			OPEN C_FUNCIONARIO
			FETCH C_FUNCIONARIO INTO @CD_CARGO,@CD_ESCOLARIDADE,@SALARIO
			BEGIN

			-- PERGAR O MAIOR E O MENOR SALARIO
				SELECT @MENOR_SALARIO = F.MENOR_VALOR,
					   @MAIOR_SALARIO = F.MAIOR_VALOR
				FROM TB_FAIXASALARIAL F
				WHERE F.CD_CARGO = @CD_CARGO
					  F.CD_ESCOLARIDADE = @CD_ESCOLARIDADE

			-- VERIFICAR SE O SALARIO DO FUNCIONARO ESTA NA FAIXA
				IF(@SALARIO<@MENOR_SALARIO OR @SALARIO > @MAIOR_SALARIO
				BEGIN
